...
* Wouter Kool
*
* plugin for displaying an online version of the demand selection task (Kool et al., 2010, JEPG)
*
**/

(function($) {
	jsPsych["dst-stim"] = (function() {
		...
		plugin.create = function(params) {
			...
			for (var i = 0; i < trials.length; i++) {
				...
				trials[i].boxes = params.boxes;
				trials[i].pairs = params.pairs;
		...
		plugin.trial = function(display_element, trial) {
			...
			var mapping = trial.mapping[block];
			var pairs = trial.pairs[block];
			var boxes = trial.boxes[block];		
			...
			var display_stimuli = function(){
				...
				$('#jspsych-dst-center-left').append($('<div id="jspsych-dst-box3"></div><div id="jspsych-dst-box5"></div>'))
				display_element.append($('<div>', {
				...
				$('#jspsych-dst-center-right').append($('<div id="jspsych-dst-box4"></div><div id="jspsych-dst-box6"></div>'))
				display_element.append($('<div>', {
					style: 'clear:both',
				}));
				display_element.append($('<div>', {
				...
				$('#jspsych-dst-box'+boxes[0]).css('background-image', 'url("img/patch'+pairs+'_0_off.png")');
				$('#jspsych-dst-box'+boxes[1]).css('background-image', 'url("img/patch'+pairs+'_1_off.png")');
			...
			var next_trial = function(){
				...
				$('#jspsych-dst-center-center').on('mouseover', function() {
					...
					$('#jspsych-dst-box'+boxes[0]).css('background-image', 'url("img/patch'+pairs+'_0.png")');
					$('#jspsych-dst-box'+boxes[1]).css('background-image', 'url("img/patch'+pairs+'_1.png")');
					...
					var patchChosen = function(){
						...
						$('#jspsych-dst-box'+boxes[unchosen]).css('background-image', 'url("img/patch'+pairs+'_'+unchosen+'_off.png")');
						$('#jspsych-dst-box'+boxes[chosen]).css('background-image', 'url("img/patch'+pairs+'_'+chosen+'_stim.png")');
